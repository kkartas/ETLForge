"""
Command-line interface for ETLTest.
"""
...
@click.option('--rows', '-r', default=100, type=int,
              help='Number of rows to generate (default: 100)')
@click.option('--output', '-o', required=True, type=click.Path(),
              help='Output file path (CSV or Excel)')
@click.option('--format', '-f', type=click.Choice(['csv', 'excel'], case_sensitive=False),
    ...
    try:
        ...
        if verbose and not result.is_valid:
            ...
            for i, error in enumerate(result.errors[:20], 1):  # Limit to first 20 errors
                ...
                click.echo(f"{i}. {error['type']} in column '{error['column']}'{row_info}")
                if error['message']:
